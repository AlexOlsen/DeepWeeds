...
inline void readJPGFile(const std::string& fileName, uint8_t* buffer, int inC, int inH, int inW)
{
    ...
    if(!file) {
       fprintf(stderr, "Error reading JPEG file %s!", fileName.c_str());
       return;
    }
    ...
    while (info.output_scanline < info.output_height) // loop
    {
      ...
      rowptr[0] = buffer +  // secret to method
